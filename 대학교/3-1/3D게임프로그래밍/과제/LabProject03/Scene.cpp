...
void CScene::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	CGameObject* pApacheModel = CGameObject::LoadGeometryFromFile(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, "Model2/M26.bin");
	...
	pApacheObject->SetPosition(50.f, 220.f, 200.f);
	m_ppGameObjects.push_back(pApacheObject);

	CGameObject* pApacheModel1 = CGameObject::LoadGeometryFromFile(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, "Model2/M26.bin");
	CApacheObject* pApacheObject1 = NULL;

	pApacheObject1 = new CApacheObject();
	pApacheObject1->SetChild(pApacheModel1, true);
	pApacheObject1->OnInitialize();
	pApacheObject1->Rotate(0.0f, 0.0f, 0.0f);
	pApacheObject1->SetScale(8.5f, 8.5f, 8.5f);
	pApacheObject1->SetPosition(150.f, 220.f, 200.f);
	m_ppGameObjects.push_back(pApacheObject1);


	CGameObject* pApacheModel2 = CGameObject::LoadGeometryFromFile(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, "Model2/M26.bin");
	...
	pApacheObject2->Rotate(0.0f, 0.0f, 0.0f);
	pApacheObject2->SetScale(8.5f, 8.5f, 8.5f);
	pApacheObject2->SetPosition(250.f, 220.f, 200.f);
	m_ppGameObjects.push_back(pApacheObject2);
...
void CScene::CheckObjectByBulletCollisions()
{
	...
	const std::vector<CBulletObject*>& bullets = m_pPlayer->GetBullets(); 
	std::vector<CApacheObject*> enemies;

	for (auto& obj : m_ppGameObjects)
	{
		CApacheObject* pEnemy = dynamic_cast<CApacheObject*>(obj);
		if (pEnemy)
		{
	...
	for (CBulletObject* pBullet : bullets)
	{
		if (pBullet && pBullet->m_bActive)
		{
			pBullet->CheckBulletCollision(pBullet, enemies);
		}
	}
}
