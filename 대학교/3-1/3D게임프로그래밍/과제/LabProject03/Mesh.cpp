	...
	int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void* pContext)
{
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++) {
		for (int x = xStart; x < (xStart + nWidth); x++, i++) {
			...
			colors.push_back(xmf4Color);  // 각 정점마다 동일한 색상 할당
		}
	}
	...
	m_pd3dColorBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, colors.data(),
		sizeof(XMFLOAT4) * colors.size(), D3D12_HEAP_TYPE_DEFAULT,
	...
	m_d3dColorBufferView.SizeInBytes = sizeof(XMFLOAT4) * colors.size();
