...
void CApacheObject::Animate(float fTimeElapsed, XMFLOAT4X4* pxmf4x4Parent)
{
	...
	//// 정적 상태: 방향, 시간 누적, 난수 생성기
	//static float elapsedTime = 0.0f;
	//static XMFLOAT3 moveDirection = XMFLOAT3(1.0f, 0.0f, 0.0f); // 초기 방향

	//static std::default_random_engine generator(std::random_device{}());
	//static std::uniform_real_distribution<float> distribution(0.0f, XM_2PI);
	...
	//	moveDirection = Vector3::Normalize(XMFLOAT3(std::cos(angle), 0.0f, std::sin(angle)));
	//	elapsedTime = 0.0f;
	//}

	//// 위치 이동
	//XMFLOAT3 displacement = Vector3::ScalarProduct(moveDirection, speed * fTimeElapsed, false);
	//XMFLOAT3 newPos = Vector3::Add(GetPosition(), displacement);
	//SetPosition(newPos);
...
void CApacheObject::Move(XMFLOAT3& vDirection, float fSpeed)
{
	SetPosition(m_xmf4x4World._41 + vDirection.x * fSpeed, m_xmf4x4World._42 + vDirection.y * fSpeed, m_xmf4x4World._43 + vDirection.z * fSpeed);
}

void CApacheObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
...
void CBulletObject::CheckBulletCollision(CBulletObject* pBullet, std::vector<CApacheObject*>& enemies)
{
	if (!pBullet->m_bActive) return;

	XMFLOAT3 bulletPos = pBullet->GetPosition();
	const float fCollisionDistance = 40.0f;

	for (auto& enemy : enemies)
	{
		XMFLOAT3 enemyPos = enemy->GetPosition();
		float distance = Vector3::Distance(bulletPos, enemyPos);

		if (distance <= fCollisionDistance)
		{
			OutputDebugString(L"[DEBUG] 거리 충돌 발생!\n");
			enemy->status = false;
			enemy->m_bBlowingUp = true;
			pBullet->Reset();
			break;
		}
	}
