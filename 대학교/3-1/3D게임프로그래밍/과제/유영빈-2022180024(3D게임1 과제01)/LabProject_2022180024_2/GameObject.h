...
public:
	...
	void Orbit(XMFLOAT3& xmf3Center, XMFLOAT3& xmf3RotationAxis, float fRotationSpeed, float fElapsedTime); // 공전
	void Scale(float sx, float sy, float sz); // 스케일 변경
...
public:
	...
	static int					EXPLOSION_DEBRISES_COUNT; // 폭발 개수
...
public:
	...
	bool IsExplosionFinished() const {
		return m_bBlowingUp && m_fExplosionTime > 1.0f; // 1초 후 제거
	}
...
{
public:
	CShieldObject() = default;
	virtual ~CShieldObject() = default;
};