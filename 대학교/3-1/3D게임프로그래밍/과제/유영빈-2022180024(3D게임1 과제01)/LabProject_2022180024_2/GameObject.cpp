...
void CGameObject::Orbit(XMFLOAT3& center, XMFLOAT3& rotationAxis, float fRotationSpeed, float fElapsedTime)
{
	// 1. 현재 위치를 불러온다
	...
	// 3. 상대 좌표를 회전한다
	XMMATRIX rotationMatrix = XMMatrixRotationAxis(XMLoadFloat3(&rotationAxis), XMConvertToRadians(fRotationSpeed * fElapsedTime));
	XMVECTOR rotatedPos = XMVector3TransformCoord(XMLoadFloat3(&pos), rotationMatrix);

	// 4. 다시 중심점으로 이동한다
	XMFLOAT3 finalPos;
	XMStoreFloat3(&finalPos, rotatedPos);
	finalPos.x += center.x;
	finalPos.y += center.y;
	finalPos.z += center.z;
...
void CGameObject::Scale(float sx, float sy, float sz) // 스케일 변경
{
	m_xmf4x4World._11 *= sx; // Right vector x
	m_xmf4x4World._12 *= sx; // Right vector y
	m_xmf4x4World._13 *= sx; // Right vector z

	m_xmf4x4World._21 *= sy; // Up vector x
	m_xmf4x4World._22 *= sy; // Up vector y
	m_xmf4x4World._23 *= sy; // Up vector z

	m_xmf4x4World._31 *= sz; // Look vector x
	m_xmf4x4World._32 *= sz; // Look vector y
	m_xmf4x4World._33 *= sz; // Look vector z
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
//
int CExplosiveObject::EXPLOSION_DEBRISES_COUNT = 10;
