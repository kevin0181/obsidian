...
CLineMesh::CLineMesh(XMFLOAT3& p0, XMFLOAT3& p1) : CMesh(1)
{
	CPolygon* line = new CPolygon(2);
	line->SetVertex(0, CVertex(p0.x, p0.y, p0.z));
	line->SetVertex(1, CVertex(p1.x, p1.y, p1.z));
	SetPolygon(0, line);
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
//
CLevel2WallMesh::CLevel2WallMesh(float fWidth, float fHeight, float fDepth, int nSubRects) : CMesh(nSubRects* nSubRects)
{
	float fHalfWidth = fWidth * 0.5f;
	float fHalfHeight = fHeight * 0.5f;
	float fHalfDepth = fDepth * 0.5f;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
//
CTankMesh::CTankMesh(float width, float height, float depth)
	: CMesh(18) // 본체 6면 + z축 아래 상자 6면 + y축 위 상자 6면
{
	float hw = width * 0.5f;
	float hh = height * 0.7f;
	float hd = depth * 0.4f;
	...
	// 본체 전면 (+Z)
	p = new CPolygon(4);
	p->SetVertex(0, CVertex(-hw, +hh, +hd));
	p->SetVertex(1, CVertex(+hw, +hh, +hd));
	p->SetVertex(2, CVertex(+hw, -hh, +hd));
	p->SetVertex(3, CVertex(-hw, -hh, +hd));
	SetPolygon(i++, p);
	...
	// z축 아래 방향 추가 상자
	float ew = width * 0.4f;
	float eh = height * 0.3f;
	float ed = depth * 0.2f;

	float baseZ = -hd - ed; // 본체보다 z축 아래로

	// 앞면
	p = new CPolygon(4);
	p->SetVertex(0, CVertex(-ew, +eh, baseZ + ed));
	p->SetVertex(1, CVertex(+ew, +eh, baseZ + ed));
	p->SetVertex(2, CVertex(+ew, -eh, baseZ + ed));
	p->SetVertex(3, CVertex(-ew, -eh, baseZ + ed));
	SetPolygon(i++, p);
	...
	// z축 아래 박스 위 Y+ 방향으로 상자 추가

	float stw = width * 0.1f;
	float sth = height * 0.5f;
	float std = depth * 0.1f;

	float baseSubY = eh + sth;        // 아래 박스 위에 놓임
	float baseSubZ = baseZ;           // z 위치는 아래 박스와 동일

	// 앞면
	p = new CPolygon(4);
	p->SetVertex(0, CVertex(-stw, baseSubY + sth, baseSubZ + std));
	p->SetVertex(1, CVertex(+stw, baseSubY + sth, baseSubZ + std));
	p->SetVertex(2, CVertex(+stw, baseSubY - sth, baseSubZ + std));
	p->SetVertex(3, CVertex(-stw, baseSubY - sth, baseSubZ + std));
	SetPolygon(i++, p);
	...
	m_xmOOBB = BoundingOrientedBox(XMFLOAT3(0, 0, 0), XMFLOAT3(hw, hh, hd), XMFLOAT4(0, 0, 0, 1));
}
