...
CCubeMeshDiffused::CCubeMeshDiffused(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, 
	float fWidth, float fHeight, float fDepth, XMFLOAT4 color) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 8;	
	m_nStride = sizeof(CDiffusedVertex);
	m_nOffset = 0;
	...
	m_pVertices[0] = CDiffusedVertex(XMFLOAT3(-fx, +fy, -fz), color);
	m_pVertices[1] = CDiffusedVertex(XMFLOAT3(+fx, +fy, -fz), color);
	m_pVertices[2] = CDiffusedVertex(XMFLOAT3(+fx, +fy, +fz), color);
	m_pVertices[3] = CDiffusedVertex(XMFLOAT3(-fx, +fy, +fz), color);
	m_pVertices[4] = CDiffusedVertex(XMFLOAT3(-fx, -fy, -fz), color);
	m_pVertices[5] = CDiffusedVertex(XMFLOAT3(+fx, -fy, -fz), color);
	m_pVertices[6] = CDiffusedVertex(XMFLOAT3(+fx, -fy, +fz), color);
	m_pVertices[7] = CDiffusedVertex(XMFLOAT3(-fx, -fy, +fz), color);
...
CTankMesh::CTankMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList,
	float fWidth, float fHeight, float fDepth, XMFLOAT4 color) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 8 * 3; // 바디 + 포탑 + 포신
	m_nStride = sizeof(CDiffusedVertex);
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	...
	// 바디 크기
	float bx = fWidth * 0.5f;
	float by = fHeight * 0.5f;
	float bz = fDepth * 0.5f;

	// 포탑 크기
	float tx = fWidth * 0.3f;
	float ty = fHeight * 0.3f;
	float tz = fDepth * 0.3f;
	float tyOffset = by + ty;

	// 포신 크기
	float gx = fWidth * 0.1f;
	float gy = fHeight * 0.1f;
	float gz = fDepth * 0.7f;
	float gzOffset = bz + gz * 0.5f;

	
	auto MakeCubeVertices = [&](int baseIndex, float x, float y, float z, float offsetX = 0, float offsetY = 0, float offsetZ = 0) {
		m_pVertices[baseIndex + 0] = CDiffusedVertex(XMFLOAT3(-x + offsetX, +y + offsetY, -z + offsetZ), color);
		m_pVertices[baseIndex + 1] = CDiffusedVertex(XMFLOAT3(+x + offsetX, +y + offsetY, -z + offsetZ), color);
		m_pVertices[baseIndex + 2] = CDiffusedVertex(XMFLOAT3(+x + offsetX, +y + offsetY, +z + offsetZ), color);
		m_pVertices[baseIndex + 3] = CDiffusedVertex(XMFLOAT3(-x + offsetX, +y + offsetY, +z + offsetZ), color);
		m_pVertices[baseIndex + 4] = CDiffusedVertex(XMFLOAT3(-x + offsetX, -y + offsetY, -z + offsetZ), color);
		m_pVertices[baseIndex + 5] = CDiffusedVertex(XMFLOAT3(+x + offsetX, -y + offsetY, -z + offsetZ), color);
		m_pVertices[baseIndex + 6] = CDiffusedVertex(XMFLOAT3(+x + offsetX, -y + offsetY, +z + offsetZ), color);
		m_pVertices[baseIndex + 7] = CDiffusedVertex(XMFLOAT3(-x + offsetX, -y + offsetY, +z + offsetZ), color);
		};

	MakeCubeVertices(0, bx, by, bz); // 바디
	MakeCubeVertices(8, tx, ty, tz, 0, tyOffset, 0); // 포탑
	MakeCubeVertices(16, gx, gy, gz, 0, tyOffset, gzOffset); // 포신
	...
	for (int i = 0; i < 3; ++i)
	{
		for (int j = 0; j < 36; ++j)
		{
			m_pnIndices[i * 36 + j] = cubeIndices[j] + (i * 8);
		}
	}
	...
	m_xmBoundingBox = BoundingOrientedBox(XMFLOAT3(0.0f, 0.0f, fDepth * 0.5f),
		XMFLOAT3(fWidth * 0.6f, fHeight * 1.2f, fDepth * 1.5f), XMFLOAT4(0, 0, 0, 1));
}
...
CSphereMeshDiffused::CSphereMeshDiffused(ID3D12Device* pd3dDevice,
	ID3D12GraphicsCommandList* pd3dCommandList, float fRadius, int nSlices, int nStacks, XMFLOAT4 color) :
