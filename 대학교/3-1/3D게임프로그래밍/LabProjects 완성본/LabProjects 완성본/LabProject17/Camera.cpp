...
void CCamera::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList
	* pd3dCommandList)
{
	UINT ncbCameraBytes = ((sizeof(VS_CB_CAMERA_INFO) + 255) & ~255); //256의 배수
	m_pd3dcbCamera = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL,
		ncbCameraBytes, D3D12_HEAP_TYPE_UPLOAD,
...
void CCamera::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	VS_CB_CAMERA_INFO* pbMappedcbCamera = (VS_CB_CAMERA_INFO*)m_pcbMappedCamera;
	memcpy(pbMappedcbCamera, &xmf4x4View, sizeof(XMFLOAT4X4));
	pbMappedcbCamera->m_xmf4x4Projection = xmf4x4Projection;
	pbMappedcbCamera->m_xmf3Position = m_xmf3Position;

	D3D12_GPU_VIRTUAL_ADDRESS d3dcbCameraGpuVirtualAddress = m_pd3dcbCamera->GetGPUVirtualAddress();
	pd3dCommandList->SetGraphicsRootConstantBufferView(1, d3dcbCameraGpuVirtualAddress);
