...
{
	//루트 시그너쳐를 생성한다. D3D12_ROOT_SIGNATURE_DESC d3dRootSignatureDesc;
...
{
	//정점 셰이더와 픽셀 셰이더를 생성한다. ID3DBlob *pd3dVertexShaderBlob = NULL;
	...
	//래스터라이저 상태를 설정한다. D3D12_RASTERIZER_DESC d3dRasterizerDesc;
	...
	//블렌드 상태를 설정한다. D3D12_BLEND_DESC d3dBlendDesc;
	...
	//그래픽 파이프라인 상태를 설정한다. D3D12_GRAPHICS_PIPELINE_STATE_DESC d3dPipelineStateDesc;
...
void CScene::Render(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	//정점 3개를 사용하여 렌더링한다. pd3dCommandList->DrawInstanced(3, 1, 0, 0);
}