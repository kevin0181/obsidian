...
bool CGameFramework::onCreate(HINSTANCE hInstance, HWND hMainWnd) {
	...
	// Direct3D 디바이스, 명령큐와 명령 리스트, 스왑 체인 등을 생성하는 함수를 호출한다.
...
void CGameFramework::CreateDirect3DDevice() {
	...
	// 다중 샘플의 품질 수준이 1보다 크면 다중 샘플링을 활성화 한다.
...
void CGameFramework::FrameAdvance() {
	...
	/* 현재 렌더 타겟에 대한 프리젠트가 끝나기를 기다린다. 프리젠트가 끝나면 렌더 타겟 버퍼의 상태는 프리젠트
	상태(D3D12_RESOURCE_STATE_PRESEND)에서 렌더 타겟 상태(D3D12_RESOURCE_STATE_RENDER_TARGET)로 바귈 것이다.*/
	...
	d3dResourceBarrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
	m_pd3dCommandList->ResourceBarrier(1, &d3dResourceBarrier);
	/* 현재 렌더 타겟에 대한 렌더링이 끝나기를 기다린다. GPU가 렌더 타겟(버퍼)을 더이상 사용하지 않으면 렌더 타겟
	...
	///* 스왑체인을 프리젠트한다. 프리젠트를하면 현재 렌더 타겟(후면버퍼)의 내용이 전면버퍼로 옮겨지고 렌더 타겟 인덱스가 바뀔 것이다. */
	m_pDXGISwapChain->Present(0, 0);

	m_nSwapChainBufferIndex = m_pDXGISwapChain->GetCurrentBackBufferIndex();

	/* 현재의 프레임 레이트를 문자열로 가져와서 주 윈도우의 타이틀로 출력한다. m_pszBuffer 문자열이 "LapProject("으로 초기화 되었으므로 (m_pszFrameRate+12)에서부터
	...
	*/
	...
	//OutputDebugString(m_pszFrameRate); // 최종 문자열 출력 
