...
//---------
{
	...
	while (true) {
		Bird bird;
		bird.read(in);
		if (in.eof()) break;  // 읽기 실패했으면 종료
		birds.push_back(bird);
	}

	cout << "읽은 Bird 객체 수는 " << birds.size() << "개 입니다\n";

	/*int numBirds = std::filesystem::file_size(inFileName) / sizeof(Bird);
	vector<Bird> birds(numBirds);

	in.read((char*)birds.data(), numBirds * sizeof(Bird));
	...
	sort(birds.begin(), birds.end(), [](const Bird& a, const Bird& b) {
		return a.getNum() < b.getNum();  // num 기준 오름차순 정렬
		});

	// 마지막 객체의 num 출력
	cout << "마지막 객체의 num 값은 " << birds.back().getNum() << "입니다." << endl;
	...
	//bool compareBirdsByNum(const Bird& a, const Bird& b) {
	//	return a.getNum() < b.getNum();
	//}

	//// [문제 3] 외부 함수를 사용한 정렬 후, 마지막 객체의 num 출력
	//sort(birds.begin(), birds.end(), compareBirdsByNum);
