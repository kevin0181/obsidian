...
//---------
{
	...
	std::cout << "읽은 Monster 객체 수 : " << v.size() << std::endl;
	...
	*/
	...
	std::ranges::sort(v, [](const Monster& lhs, const Monster& rhs) {
	...
	std::ranges::sort(v, [](const Monster& lhs, const Monster& rhs) {
		return std::string(const_cast<Monster&>(lhs).getS())
			< std::string(const_cast<Monster&>(rhs).getS());
		});
	v.back().show();
	...
	for (Monster& m : v) {
		...
		if (s.size() >= 9 && std::ranges::equal(s, std::string{ "123456789" }))
			++count;
	}
	std::cout << "1~9까지 모든 숫자를 가진 Monster 수: " << count << std::endl;
