...
//---------
{
	...
	std::cout << "Dog 객체의 개수는 " << dogNum << "개입니다" << std::endl;
	...
	std::vector<Dog> v(dogNum);
	in.read(reinterpret_cast<char*>(v.data()), sizeof(Dog) * dogNum);
	std::cout << "마지막 Dog 객체의 num 값은 " << v.back().getNum() << "입니다" << std::endl;
	...
	*/
	...
	int compareCount{};
	std::ranges::sort(v, [&](const Dog& lhs, const Dog& rhs) {
		++compareCount;
		return lhs.getNum() < rhs.getNum();
		});
	std::cout << "비교함수 호출 횟수 : " << compareCount << std::endl;
	...
	auto it = std::ranges::max_element(v, [](const Dog& lhs, const Dog& rhs) {
		double avg_lhs = std::accumulate(lhs.getData(), lhs.getData() + lhs.getNum(), 0.0) / lhs.getNum();
		double avg_rhs = std::accumulate(rhs.getData(), rhs.getData() + rhs.getNum(), 0.0) / rhs.getNum();
		return avg_lhs < avg_rhs;
		});
	std::cout << "평균값이 가장 큰 Dog 객체의 num 값은 " << it->getNum() << "입니다" << std::endl;
	...
	for (int i = std::numeric_limits<int>::min(); i != std::numeric_limits<int>::max(); ++i) {
		totalByte += std::to_string(i).size();
		if (i == -1) break; // 범위 오버플로우 방지
	}
	++totalByte; // std::numeric_limits<int>::max() 추가
	double average = static_cast<double>(totalByte) / (1ull << 32); // 총 개수: 2^32
	std::cout << "평균 바이트 수: " << std::fixed << std::setprecision(2) << average << std::endl;
