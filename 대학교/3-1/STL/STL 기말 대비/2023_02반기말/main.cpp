...
//---------
{
	...
	auto all_digits_odd = [](int n) {
		n = abs(n); // 음수 처리
		string s = to_string(n);
		return ranges::all_of(s, [](char c) { return (c - '0') % 2 == 1; });
		};

	int odd_digit_count = count_if(v.begin(), v.end(), all_digits_odd);
	cout << "모든 자릿수가 홀수인 수 개수: " << odd_digit_count << endl;
